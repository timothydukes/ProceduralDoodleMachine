# PDM_SPEC v0.3 — Procedural Doodle Machine
...
Status: v0.3 — fully approved, no open items. Changes from v0.2: renderer
P3D approved (interpretive flag removed), and the performance-floor policy
amended per author direction (provisional 30 fps, per-style negotiable).
...
- Renderer: **P3D** (locked; approved). Phase 0 measured P2D and P3D as
  equivalent on the Pi and JAVA2D as unusable; P3D selected because one
  planned style family is explicitly 3-D (one-point perspective mode).
  Cost of reversal to P2D later: one line plus per-style retesting.
...
- Performance: **30-60 fps is the target range.** The floor is
  **provisionally 30 fps** and is a per-style negotiable, not an absolute:
  a computationally expensive, slow-moving style may declare a floor as low
  as 10-15 fps if it looks right at that rate. The declared floor is set
  during that style's study phase and encoded in its `fpsFloor()`. Phase 0
  budgets (§3.1) were measured against 30 fps; a style trading down to a
  lower floor buys proportionally more headroom against those ceilings.
...
- **Phase 0 — Pi benchmarking. COMPLETE.** Results and analysis:
  ...
  Outcomes locked into §3/§3.1: renderer P3D (approved), 720p
  internal, target/floor policy per §3, one-light-shader-pass GLSL policy, complexity
...
*PDM_SPEC v0.3 — supersedes v0.2. No open items.*
